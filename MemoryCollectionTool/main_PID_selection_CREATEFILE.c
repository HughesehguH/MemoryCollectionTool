...
BOOL getProcessInfo(HANDLE hProcessSnap, unsigned int PID)
{
   ...
   // and exit if unsuccessful
    ...
    // match_found = FALSE;
    while (pe32Parent.th32ProcessID != PID)
    {
        Process32Next(hParentProcessSnap, &pe32Parent);
    }
    //if (match_found)
    //{
        ...
        if (Toolhelp32ReadProcessMemory(pe32.th32ProcessID, me32.modBaseAddr, &buff, (BUFF_SIZE * sizeof(DWORD)) - 3, &cBytesRead) == TRUE)
        {
            ...
            fp = CreateFileA("TESTING.txt", GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
            if (fp != INVALID_HANDLE_VALUE)
            {
                if (WriteFile(fp, &buff, (BUFF_SIZE * sizeof(DWORD)) - 3, &bytesWritten, NULL))
                {
